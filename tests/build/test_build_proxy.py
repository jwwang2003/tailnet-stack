...
class BuildProxyTests(unittest.TestCase):
    def run_build(self, proxy=None, endpoint='unix:///var/run/docker.sock', driver='docker', rootless=False, platform=None):
...
''')
            ...
            env = {k:v for k,v in os.environ.items() if k not in ('BUILD_PROXY_URL','DOCKER_HOST','DOCKER_CONTEXT','BUILD_PLATFORM')}
