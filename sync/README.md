...
`issuer` must equal the Casdoor OIDC issuer Headscale is configured with: Headscale's provider identifier is `issuer/subject`, and the worker matches it against `issuer/<Casdoor user id>`. `on_block` is `expire` (default) or `delete`; `revoke_preauth_keys` defaults to true. When a full applied run sets `isForbidden: true` on a user (inactive status or confirmed missing), the worker looks up every Headscale user whose `providerId` equals that value, expires (or deletes) each of their nodes through the Headscale REST API, and expires their preauth keys. Reports carry `revocation: {configured, targets, revoked, pending, headscale_users, nodes_expired, nodes_deleted, preauth_keys_expired}` in every synchronization and offboarding mode; a dry run counts the targets and touches nothing.

Revocation is durable. Before a run sends a blocking `isForbidden: true` write, it records the Casdoor subject in the journal `<state_file>.revocations.json` (`{"version": 1, "pending": {"<casdoor id>": {"since": <unix time>, "source": "sync" | "offboard"}}}`, written atomically like the state file). After Headscale confirms, the worker writes `properties.feishu_sync_revoked: "<unix time>"` on the user through `update-user?columns=properties` and removes the journal entry. Every applied run, full or staged, revokes the journaled subjects plus every linked user that is forbidden with `feishu_sync_forbidden` or `feishu_sync_hold: "true"` and carries no `feishu_sync_revoked` marker, so a crash, a lost write response, or a failure later in the same run cannot lose a revocation. A journal entry whose user is not forbidden (the block never applied) is dropped without a Headscale call, and a user blocked by an administrator without either marker is never revoked automatically. Re-enabling with `allow_reenable` clears the revoked marker together with the block marker, and a new block by the worker or by `--offboard` clears a stale revoked marker in the same write, so an account that was re-enabled by hand and blocked again is found even if the journal was restored from an older backup. A journaled block whose `feishu_sync_forbidden` marker was removed by hand is still revoked: the journal is the worker's own record that it applied that block. The first applied run after upgrading from rc.2 revokes every user that rc.2 had blocked once more (idempotent: already expired nodes and keys stay expired) and writes the marker.

If Headscale is unreachable or answers with a malformed response, the Casdoor block stays applied, the journal entry is kept, the state file is saved, the run ends with an error, and the revocation is retried on every following applied run until it succeeds. If `headscale` is omitted, `revocation.configured` is false, `pending` counts the subjects whose devices still need the manual runbook, the run does not fail, and the journal entries are kept for a later configuration. A corrupt journal stops every synchronization mode (dry run, `--apply`, staged runs and `--offboard`) until it is restored from backup; `--audit`, `--permissions` and `--profile-only` do not read it. Journal entries kept while `headscale` is unset are retired automatically by the first applied run after the section is configured. State files written by rc.2 carry `pending_revocations`; the list is moved into the journal on the next run and written back empty.

Applied runs hold the advisory lock `<state_file>.lock` only while a run is in progress; `--watch` releases it between intervals and additionally holds `<state_file>.watch.lock` for its lifetime, so a second continuous worker is refused while `--offboard --apply` can interleave. `lock_wait_seconds` (default 300, 1 to 3600) bounds how long an offboarding waits for a running interval.
...
`--offboard employees/NAME` prints the planned Casdoor changes; `--offboard employees/NAME --apply` takes the state lock (waiting up to `lock_wait_seconds` for a running interval), journals the subject with `source: offboard`, blocks the account (`isForbidden: true`), sets `properties.feishu_sync_hold: "true"` so no later run re-enables it, sets `headplane_role` to `member`, removes the worker-managed groups (the admission alias and the `feishu-*` groups; local groups are preserved), and, when `headscale` is configured, revokes the user's nodes and preauth keys immediately and records `feishu_sync_revoked`. If Headscale fails, the command exits with an error, the block and the journal entry remain, and the next scheduled applied run completes the revocation. The target must be `organization/name` inside the configured organization. The report lists `casdoor_updates` and `revocation`; the command is one-shot and touches only the journal and the lock, never the state file. Operators still review routes, API keys, Casdoor sessions, and Headplane sessions per the operations runbook. Clear the hold only after the review that would justify re-enabling.
